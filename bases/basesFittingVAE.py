...
# TODO : utiliser ray pour hyper parameter opt
config_fn = "/UR10_Braccio.yaml"
...
bases_r2.in_.load_state_dict(torch.load("models/{}/bases_dim_{}/100k/{}/{}_input.pth".format(PRIMITIVE_TASK,DIM_LATENT,run_id,R2_NAME)),strict = True)
bases_r2.out.load_state_dict(torch.load("models/{}/bases_dim_{}/100k/{}/{}_output.pth".format(PRIMITIVE_TASK,DIM_LATENT,run_id,R2_NAME)),strict = True)
...
for epoch in range(num_epochs):
    for batch_idx, (s1, s2) in enumerate(train_dataloader):
        ...
        # compute similarity loss i.e MSE(latent_r1,latent_r2)
        loss_similarity = GAMMA*similarity_crit(z_r1, z_r2)
        ...
        loss_cross_r1 = reconstruction_crit(y_cross_r1,s1) * DELTA
        ...
        total_loss = loss_reconstruction_r1  + loss_similarity + loss_KL_r1  + loss_cross_r1 
    ...
    print("sim_loss : ", sim_loss/GAMMA/len(train_dataloader))
    ...
    print("cross_loss_1 : ", cross_loss_1/DELTA/len(train_dataloader))
    KL_loss_1 = 0
    recon_loss_1 = 0
    sim_loss = 0
    cross_loss_1 = 0
...
# afficher erreur de reconstruction total sur le dataset pour chaque robot + erreur de sim
...
print("std r1 :",np.sqrt(np.std(mu1,0)))
print("std r2 :",np.sqrt(np.std(mu2,0)))
...
targets = ['latent space UR10 (sampled)','latent space Braccio (sampled)','latent space UR10 (mu)','latent space Braccio (mu)']
